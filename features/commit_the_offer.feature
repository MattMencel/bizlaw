Feature: A Team commits its Offer and the exchange half pays for it
  Committing an Offer is executing a draft, at most one per Side per Day — which
  closes the table at both ends, so a Day whose Offer is executed takes neither a
  second commit nor a new draft. It costs one point of the exchange half — the half that buys an Offer and the
  ...
  Scenario: An Offer lands on a flat Day, and a second one cannot follow it
    ...
    And drawing a new plaintiff Offer on Day 1 is refused
    And the plaintiff Side has 7 preparation points and 1 exchange points left on Day 1
