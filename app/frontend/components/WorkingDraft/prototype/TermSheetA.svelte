<!--
  PROTOTYPE — variant A, the control: `TermSheet.svelte` exactly as it ships.

  Four columns, Term / Ours / Theirs / What she asked for. Here so B and C are
  judged against the thing they are trying to beat rather than against a memory
  of it, and so the three silences can be compared side by side: this renders
  "a Term nobody tabled" and "a Position with no figure" as the same grey
  italic `silent`, and "a Client with no aspiration" as an em dash.
-->
<script>
