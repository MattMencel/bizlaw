...
# neither could an Instructor's waiver, nor the Acceptance that ends the
...
class Docket
  ...
  # several ledgers written inside one test — or one fast Day — can share a
  # timestamp. Rows of one ledger then keep the order that ledger returned them
  # in, and every one of these relations orders by `id`, so that order is the
  ...
  # Taking the Offer across the table, attributed to the member who took it.
  # Committing one is a spend and is already above; this is the other half of
  # the deal and costs nothing.
  def acceptances
    scoped(side.offer_acceptances).map do |acceptance|
      line(
        at: acceptance.created_at,
        act: OFFER_ACCEPTED,
        by: acceptance.accepted_by,
        day: acceptance.day
      )
    end
  end
