...
- **Invariants live in the database wherever they fit in one** — a Second as a CHECK, an Exhibit playable once by unique index on the Case File row it is spent from, a shift landing once per document by a *partial* unique index over the receiving Side's Case File row — partial because `source_ref` names no source table and an Event's shift will point elsewhere — tenancy as composite foreign keys — and where the boundary is narrower than tenancy, the narrower key: an Exhibit rides only its own Team's Offer because `staged_offer_exhibits` reaches both parents by `(id, side_id)`, which two Sides sharing a Simulation and an Organization would otherwise slip past. A run's own tables key on `(parent_id, simulation_id, organization_id)`, not the Organization alone: a Section runs many concurrent Simulations, so the narrower key would permit a row pairing a Side from one run with a Day from another. Rules that span tables stay in Ruby.
