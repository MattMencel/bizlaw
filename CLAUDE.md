...
Node is build-time only. It appears in `assets:precompile` (enhanced by vite_ruby, never replaced) and in `bin/vite dev`, and the pin lives in `.node-version`, mirrored in CI and the Dockerfile.
