...
An Offer is **staged** before it is committed: visible to the whole Team, revisable, costing nothing until it lands. A staged Offer is a **draft** on the surface, marked unexecuted, so a Team can tell a position it is deliberating from one it has taken; committing one is **executing** it. Executing closes the table for that Day at both ends — a Team commits at most one Offer a Day, and a Day whose Offer is executed takes no new draft either. A draft written after it could never be committed and cannot be carried forward, and the surface reads the draft as the Team's position, so one written there would leave the executed instrument showing terms nobody countersigned.
...
**The other Side's paper is a block of its own on the front.** Their last committed Offer reaches a Team as the redline on its own term sheet and in no other form — it is not in the Case File, which answers *what do we know* — so an Acceptance, which is a countersignature on their instrument, needs something to attach to. What it attaches to names the instrument rather than restating it: the Day it was committed on, the hand that drew it, and their covering note. The terms are the strike on the sheet above; printing them twice would make the reader compare two copies of one position.

**The Instructor's paper is the minute.** They are not in the dispute, so what they are handed is not a file: the Section in the letterhead rather than the matter, and one ruled line per Side for the Day being played, carrying the one act an Instructor takes inside a running Day. It says of a Team only whether a position is on the table with nobody's countersignature under it, which is the whole of what a waiver would change; the rest of what they see of a run belongs to their own surfaces and not to this one. It bears **no signature line** — a block modelled on the countersignature it releases would invite the Instructor to sign for a Team, which is the one thing the Second refuses. Once the run has settled it is one line — the Day the matter closed, and that there is nothing left to release. Their powers are over a running Day and a settled run has none.
...
`:prose` is Student Prose. `:skeleton` is the graded shape: Docket, committed Offers, outcome, scores, revealed ranges and Attribution. `:authored` is Case rows and their pinned Case Versions — the professor's work, never a student record, never purged, and the generated Client lines are among them: a line is written against the Case rather than against any run. Which one a Client spoke is stored nowhere — it is folded from the seed and that Consult's own Docket row — so a memo lasts exactly as long as the skeleton row it hangs off and is purged with it.
