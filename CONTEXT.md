...
An Offer is **staged** before it is sent: visible to the whole Team, revisable, costing nothing until it lands. A staged Offer is a **draft** on the surface, so a Team can tell a position it is deliberating from one it has taken. A draft is not yet an offer and is not *on the table*, because the other Side cannot see it; **sending** it is what makes it one. An Offer is sent, never *executed* — execution binds, and an Offer binds nobody until it is accepted, so *executed* belongs to the executed instrument alone. See ADR 0009. Sending closes the table for that Day at both ends — a Team sends at most one Offer a Day, and a Day whose Offer is sent takes no new draft either. A draft written after it could never be sent and cannot be carried forward, and the surface reads the draft as the Team's position, so one written there would leave the sent Offer showing terms nobody countersigned.
...
**There are no rooms.** A Team does not go anywhere to prepare or anywhere to deal; it works one file. What used to be two places is carried by the instrument instead: a spend is drawn from the Action Board, and an Offer is sent from a draft over a countersignature. No surface is anywhere else, so no movement between them can be mistaken for a commitment — the Second is the only gate a Team has.

**The instrument has two faces.** The front is the **working draft**: the Morning Briefing as front matter, scrolled past rather than dismissed; the term sheet beneath it, which is where the Terms Board's three tracks live — the Team's own position, the other Side's last sent Offer in redline, the Client's aspiration in the margin; the countersignature block permanently under it; what is clipped to the draft down the side; and the Action Board a slip along the foot. The back is the **Case File and the Docket**, on the reverse of the same instrument rather than surfaces of their own — the file is turned over, never navigated away from. The known cost is that a surface behind a gesture is one a Team may never turn to; if nobody turns the page, that is a finding rather than a defect to design around.
...
Where the shape of the deal is visible: each Term as a track carrying the Team's own position, the other Side's **last sent** Offer, and the Team's own Client's stated aspiration. Reachable whenever a Team is working, because deliberating over the shape of a deal is preparation work even though sending one is not.
...
There are two kinds. A **Consult** node is keyed to the Reaction Band and carries several variants, chosen by the Simulation seed *and* how many times the node has already been spoken, so a Client consulted on five Days does not repeat itself. A **settlement** node is keyed to which Side accepted — took it, or had it taken, which are different feelings about identical terms — and carries no variants at all: the acceptance role is the whole of what the occasion varies by. It is spoken exactly once, so a speak-count never advances there and the seed alone would choose; one line per role is what a Case authors, because the role is *what happened* where a variant is the same feeling said again.
...
`:prose` is Student Prose. `:skeleton` is the graded shape: Docket, sent Offers, outcome, scores, revealed ranges and Attribution. `:authored` is Case rows and their pinned Case Versions — the professor's work, never a student record, never purged, and the generated Client lines are among them: a line is written against the Case rather than against any run. Which one a Client spoke is stored nowhere — it is folded from the seed and that Consult's own Docket row — so a memo lasts exactly as long as the skeleton row it hangs off and is purged with it.
...
A **demo Section** is an ordinary Section of the same Case, seeded, with no real students in it — what an Instructor teaches a room of first-timers from. Nothing in the engine knows it is a demo: no flag, no mode, no exclusion. It is created and re-created by a rake task alongside `case:import`, which resets its own Organization and is re-run over a Day already played rather than refusing to touch one. Every act it lays down goes through the **same seams a student's act does**, in the Day's own order, rather than being written as fixture rows — a seed that wrote rows directly would stop being evidence the engine works, and would drift from the runtime schema silently. A Section aged out under Retention's clocks is re-seeded rather than preserved, and the same artifact is the development fixture, which is what earns building it.

It lays down **two runs, not one**. The first sits on the earliest Day that shows the record surfaces full, which is arithmetic rather than preference: a Day showing a served Exhibit cannot fall before the lead times allow one. A live Simulation is no substitute for it — at the moment a class is watching, a real run's Docket is empty. The second is the **cold open**, Day 1, where every record surface is empty on purpose. That is not a degenerate case to be avoided but the thing Onboarding claims — the empty state is the tutorial — and a demo that skipped it would show only half the teaching.
